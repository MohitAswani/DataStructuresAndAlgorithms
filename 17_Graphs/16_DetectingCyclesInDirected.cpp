...
of 2.
 * So cycles are present in directed graphs if in the same recursive call of DFS_helper function we detect a vertices which we alreaddy visited in the  
...
bool cycleInDirected(vector<int> adj[], int v)
{
    ...
    {
        if(visited[i]==false)
            return DFS_helper(adj,i,visited,recCallStack);  
    }
