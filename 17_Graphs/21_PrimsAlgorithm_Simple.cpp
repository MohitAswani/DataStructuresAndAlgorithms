 ...
 * PRIM'S ALGORITHM : * In this algorithm we divide the vertices in two sets.
                      ...
                      * So first we select any vertex and find the edge with minimum weight connecting the vertices in MST 
                      to ones not in MST.
...
int primMST(ll graph[V][V])
{
    ...
    // We can make the time complexity of this algorithm = O((V+E)logV) by using a adjacency list and a priority_queue for key.
