...
// FOR BETTER UNDERSTANDING WATCH THE VIDEO "Coin Change Count Combinations"
/**
 ...
 * for tabulation.
 *
 * In each cell of the array we are storing the number of ways to make the sum using the coins.
 * Each cell dp[i][j] represents the number of ways to make sum i using the first j coins.
 *
 * For each cell we are first adding the cases to make the given sum using first j-1 coins. Then
 * we check for cases which can be made using the first j coins. (Same logic as recursive solution but
 * we are using tabulation approach).
 *
...
int coinChange(int coins[], int size, int sum)
{
    ...
    /*
     The above array cannot be dp[size+1][sum+1] because we will add the same cases again and again
     as for in the 2D loop we will go through same sum for every coin which will include the previous
     cases too hence we will get extra cases.
    */
    ...
    for (int i = 1; i <= sum; i++) // this represents the current requirred sum
    {
        for (int j = 1; j <= size; j++)
        {
            ...
            if (i>=(coins[j - 1]))
            {
                dp[i][j] += dp[i - coins[j - 1]][j]; // cases in which the current coin is included
