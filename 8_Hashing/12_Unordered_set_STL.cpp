 ...
 * size(),empty()  (O(1))
 * 
 * In hashing we assume the keys get uniformly distributed so each hash will have constant keys associated with it hence  these operations occurs in constant time.
 * 
 * ****************APPLICATIONS*******************
 * Unordered sets are implement using hashing and hence it can perform search,insert,delete in O(1) time average and hence can be used anywhere where we need this kind of functionality on keys.
 * 
 * 
 * @return int 
 */
