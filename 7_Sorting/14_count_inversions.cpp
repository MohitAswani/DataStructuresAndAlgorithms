 ...
 * @brief
 *
 * We add n1-i to res if right<left since all the elements are sorted so rest of the elements of left array will > current right element.
 * 
 * So all the element greater than left[i] will form inversion with right[i].
 * 
 * And also this is the advantage of sorting since we can easily find the number of elements forming inversion without even iterating.
 * 
 * So this will count all the inversions in which one is in the left and other in right but since it counts for all l and r so it does it for every element.
 * 
