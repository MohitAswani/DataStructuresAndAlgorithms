 ...
 * @brief
 *
 * The following function splits the array into two arrays , sorts them and then merge these two sorted arrays.
 *
 * Merge sort is stable.
 *
 * Time complexity : O(nlogn) (If we see the tree of merge sort we find that at each level the work done is theta(n) and there are theta(logn) levels so the overall time complexity is theta(nlogn))
 *
 * Auxillary space: O(n) (In each step is theta(n) but overall auxillary space is also same since after each function call returns space is deallocated and hence at a particular time max space occupied in ram is theta(n))
 *
 * @param arr
 * @param l
 * @param r
