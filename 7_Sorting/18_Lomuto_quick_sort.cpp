...
4)Despite O(n^2) it is faster than merge sort as it is:
    a)In-place (lomuto&hoare's): that means it does not create temporary arrays for partition but only uses func call stack for recursion.
    b)Hence cache friendly so it is considered faster than merge sort.
    c)Average case is O(nlogn).
    d)Tail recursive : but merge was not tail recursive since last thing was merge func not recursive call.(Tail recursive function will be converted to iterative loop by compiler optimisation)
...
(which uses intrasort variation of quick sort) and stable sort . But some lang like python and java use timsort which is a variation of merge sort. So for stability merge sort is preferred.
*/
#include <bits/stdc++.h>
using namespace std;
